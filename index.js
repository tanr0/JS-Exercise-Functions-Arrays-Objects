...
function getLastCarInfo(carsArray) {
  const arrayLength = carsArray.length -1;
  return 'This is a ' + carsArray[arrayLength].car_make + carsArray[arrayLength].car_model;
  }
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(carsArray, carID) {
  for (let i=0; i < carsArray.length; i++){
  
    return `This is a  ${carsArray[i].car_make} ${carsArray[i].car_model}`
  }
}
...
return carsArray.sort((car1, car2) => {
...
function getModelYears(carsArray) {
  let carYears = [];
  for(let i=0; i<carsArray.length; i++){
  carYears.push (carsArray[i].car_year);    
  }
return carYears;  
}
...
function getOlderCars(carsArray, maxYear) {
  let oldCars= [];
  for(let i=0; i<carsArray.length; i++){
    if (carsArray[i].car_year <= maxYear){ oldCars.push (carsArray[i]);
    }  
  }
  return oldCars;
}
